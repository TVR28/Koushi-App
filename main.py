...
st.markdown("<h1 style='text-align:center; padding:10px; font-size:80px; color:#FF4B4B'>Koushi App</h1>",unsafe_allow_html=True)
st.markdown("---")
...
if file is not None:
    ...
    if st.session_state.count >= 1:
        st.image(f'page{st.session_state.count}.jpg', caption=f'Page{st.session_state.count}',width = 500)
        audio_file_en = open(f'page{st.session_state.count}_en.mp3', 'rb')
        audio_bytes_en = audio_file_en.read()
        audio_file_te = open(f'page{st.session_state.count}_te.mp3', 'rb')
